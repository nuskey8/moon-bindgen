...
fn moon_type(ty: &Type, model: &Model, rename: fn(String) -> String) -> Option<String> {
    match resolve_alias(ty, model) {
        ...
        Type::Path(path) => Some(match last(path) {
            ...
            "bool" => "Bool".into(),
            "c_void" => "Unit".into(),
            name if crate::generate::c_scalar_moon_type(name).is_some() => {
                crate::generate::c_scalar_moon_type(name).unwrap().into()
            }
        ...
        Type::Pointer { inner, mutable } => Some(format!(
            ...
            moon_pointee_type(inner, model, rename)?
        )),
        Type::Array { inner, .. } => {
...
fn moon_pointee_type(ty: &Type, model: &Model, rename: fn(String) -> String) -> Option<String> {
    match resolve_alias(ty, model) {
        Type::Path(path) => Some(
            crate::generate::c_marker_type(last(path))
                .map(str::to_owned)
                .unwrap_or_else(|| type_name(last(path), rename)),
        ),
        other => moon_type(other, model, rename),
    }
}
...
fn primitive_c_type(name: &str) -> Option<&'static str> {
    match name {
        ...
        "i64" => Some("int64_t"),
        "u64" => Some("uint64_t"),
        "isize" | "ptrdiff_t" => Some("ptrdiff_t"),
        "usize" | "size_t" => Some("size_t"),
...
fn default_value(ty: &Type, model: &Model) -> Option<String> {
    match resolve_alias(ty, model) {
        Type::Path(path) => match last(path) {
            "i8" | "u8" => Some("Byte::default()".into()),
            "i16" | "u16" | "i32" => Some("Int::default()".into()),
            "u32" => Some("UInt::default()".into()),
            "i64" => Some("Int64::default()".into()),
            "u64" => Some("UInt64::default()".into()),
            "f32" | "c_float" => Some("Float::default()".into()),
            "f64" | "c_double" => Some("Double::default()".into()),
            "bool" => Some("Bool::default()".into()),
            name => crate::generate::c_scalar_moon_type(name).map(|ty| format!("{ty}::default()")),
        },
        _ => None,
    }
}
